<template>
  <div v-if="mostrar" class="modal-overlay">
    <div class="modal-content">
      <h2 class="text-2xl font-bold mb-4">{{ modo === 'crear' ? 'Crear Nueva Categoría' : 'Editar Categoría' }}</h2>
      <form @submit.prevent="handleSubmit">
        <div class="form-group">
          ...
          <input 
            type="text" 
            id="nombre_categoria" 
            v-model="formularioLocal.nombre_categoria" 
            required 
...
const props = defineProps({
  ...
  categoriaData: {
    type: Object,
    default: () => ({ nombre_categoria: '', descripcion: '' })
  }
});
...
watch(() => props.categoriaData, (newData) => {
  formularioLocal.value = { ...newData };
}, { deep: true, immediate: true });

const handleSubmit = () => {
  if (!formularioLocal.value.nombre_categoria) {
    errorFormulario.value = 'El nombre de la categoría es obligatorio.';
    return;
  }
