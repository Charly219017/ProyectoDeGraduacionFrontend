...
// frontend/src/componentes/Mantenimiento/DependenciasTabla.vue
<template>
  <div class="dependencias-card">
    <h2 class="dependencias-title">Lista de Dependencias</h2>
    <div v-if="cargando" class="dependencias-empty">Cargando dependencias...</div>
    <div v-else-if="dependencias.length > 0" class="dependencias-table-container">
    ...
    <div v-else class="dependencias-empty">
...
.dependencias-table th, .dependencias-table td {
  padding: 14px 18px;
  text-align: left;
}
...
.dependencias-table tr {
  transition: background 0.2s;
}

.dependencias-table tbody tr:hover {
  background: #f3f6fd;
}
