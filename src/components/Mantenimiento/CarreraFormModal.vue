...
// frontend/src/componentes/Mantenimiento/CarreraFormModal.vue
<template>
  <div v-if="mostrar" class="modal-overlay">
    <div class="modal-content">
      <h2 class="text-2xl font-bold mb-4">{{ modo === 'crear' ? 'Crear Nueva Carrera' : 'Editar Carrera' }}</h2>
      <form @submit.prevent="handleSubmit">
        <div class="form-group">
          ...
          <input 
            type="text" 
            id="nombre_carrera" 
            v-model="formularioLocal.nombre_carrera" 
            required 
...
watch(() => props.carreraData, (newData) => {
  formularioLocal.value = { ...newData };
}, { deep: true, immediate: true });

const handleSubmit = () => {
  if (!formularioLocal.value.nombre_carrera) {
