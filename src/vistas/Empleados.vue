...
import EmpleadosList from '../componentes/Empleados/EmpleadosList.vue';
import EmpleadoFormModal from '../componentes/Empleados/EmpleadoFormModal.vue';

// Importa todas las funciones del servicio centralizado
import { 
...
const guardarEmpleado = async (datosEmpleado, modo) => {
  ...
  try {
    if (modo === 'crear') {
      ...
      await crearEmpleado(datosEmpleado, usuarioIdAuditor);
    } else {
      // **AUDITORÍA**: Pasamos el ID del usuario auditor al servicio
      await actualizarEmpleado(datosEmpleado.id_empleado, datosEmpleado, usuarioIdAuditor);
    }
...
const confirmarEliminar = async () => {
  ...
  try {
    // **AUDITORÍA**: Pasamos el ID del usuario auditor al servicio
    await eliminarEmpleadoApi(empleadoAEliminar.value, usuarioIdAuditor);
    await obtenerDatosIniciales();
  } catch (error) {
