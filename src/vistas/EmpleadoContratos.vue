...
import ContratosList from '../componentes/Contratos/ContratosList.vue';
import ContratoFormModal from '../componentes/Contratos/ContratoFormModal.vue';
import { 
  obtenerContratosPorEmpleado, 
  crearContrato, 
  actualizarContrato, 
...
const guardarContrato = async (datosContrato, modo) => {
  ...
  try {
    if (modo === 'crear') {
      await crearContrato(datosContrato, usuarioActualId);
    } else {
      await actualizarContrato(datosContrato.id_contrato, datosContrato, usuarioActualId);
    }
    await obtenerDatos();
    cerrarModal();
  } catch (error) {
...
const eliminarContrato = async (id) => {
  if (window.confirm('¿Estás seguro de que deseas eliminar este contrato?')) {
    const usuarioActualId = authStore.usuario?.id_usuario;
    try {
      await eliminarContratoApi(id, usuarioActualId);
      await obtenerDatos();
    } catch (error) {
